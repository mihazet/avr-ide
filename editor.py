...
class EditorPage(wx.stc.StyledTextCtrl):
    ...
    def InsertSeperator(self):
        """Insert a seperator comment"""
        dlg = wx.TextEntryDialog(self, 'Enter seperator label text:',
                    'Insert seperator', '')
        if dlg.ShowModal() == wx.ID_OK:
            ...
            string = (' '*indent)+'#---'+label+('-'*(80-len(label)-4-indent))+'\n'
            line   = self.LineFromPosition(pos)
            pos    = self.PositionFromLine(line)
            self.InsertText(pos,string)
        dlg.Destroy()
...
About Us (Contact Us) | Privacy | Twitter | Facebook | Feedback
